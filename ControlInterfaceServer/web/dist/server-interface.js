...
var Utils = /** @class */ (function () {
    ...
    Utils.getEffectsList = function (manageLoader) {
        if (manageLoader === void 0) { manageLoader = false; }
        if (manageLoader)
            Utils.showLoader();
        return new Promise(function (resolve, reject) {
            $.getJSON('/manager/list').done(function (data) {
                if (manageLoader)
                    Utils.hideLoader();
    ...
    ;
    Utils.getScheduledEventsList = function (manageLoader) {
        if (manageLoader === void 0) { manageLoader = false; }
        if (manageLoader)
            Utils.showLoader();
        return new Promise(function (resolve, reject) {
            $.getJSON('/scheduler/list').done(function (data) {
                if (manageLoader)
                    Utils.hideLoader();
                if (data) {
                    resolve(data);
    ...
    ;
    Utils.triggerException = function (msg) {
        if (msg === void 0) { msg = ''; }
        throw msg || '';
    };
    ...
    Utils.objectifyForm = function (formArray) {
        var returnArray = {};
...
Object.defineProperty(exports, "__esModule", { value: true });
var ScriptManager_1 = __webpack_require__(3);
var ScriptScheduler_1 = __webpack_require__(5);
var Utils_1 = __webpack_require__(0);
var define = define || undefined;
var module = module || undefined;
var ScriptMain = /** @class */ (function () {
    function ScriptMain() {
    }
    ScriptMain.init = function () {
        Utils_1.default.init();
        ScriptManager_1.default.init();
        ScriptScheduler_1.default.init();
    };
    return ScriptMain;
}());
exports.default = ScriptMain;
(function () {
    ScriptMain.init();
})();
    ...
    ScriptManager.init = function () {
        ...
        $(document).ready(function () {
            var template = "<li data-id=\"{{ id }}\">\n" +
                ...
                "            </div>\n" +
                "        </li>";
            var $listElt = $("ul#effects-list");
            var $modalElt = $("section#modal");
            var $colorPickerWindow = $("section#color-picker");
            var $btnCreateNew = $("button#create");
            var $btCloseModal = $modalElt.find("div.close");
            var $btnSave = $modalElt.find("button#saveBtn");
            var $btnCancel = $modalElt.find("button#cancel");
            var $resetBtn = $("#reset");
            var $colorPickerBtn = $("#btncolorpicker");
            var form = $("#effect-form")[0];
            var canvas = $(".canvas-wrapper > canvas")[0];
            var $btnWhiteCanvas = $("#btnWhiteCanvas");
            var $btnColorCanvas = $("#btnColorCanvas");
            var $btnCloseColorPicker = $("#closeColorPicker");
            var $pickerCtrl = $("#picker-ctrl");
            var $previewColorElt = $("#preview-color");
            var persistentEffectsClass = "persistent";
            var clearList = function () {
                $listElt.find('> li:not(.' + persistentEffectsClass + ')').remove();
            };
            ['mousemove', 'touchmove'].forEach(function (event) {
                $modalElt.on(event, function (e) {
                    e.preventDefault();
                });
            });
            var showModal = function (id, effect) {
                if (id === void 0) { id = NaN; }
                ...
                Utils_1.default.$mainSectionElt.addClass('blur');
                $modalElt.addClass('visible');
            };
            var hideModal = function () {
                Utils_1.default.$mainSectionElt.removeClass('blur');
                $modalElt.removeClass('visible');
            };
            var refreshEffectsList = function () {
                // Utils.showLoader();
                Utils_1.default.getEffectsList().then(function (data) {
                    clearList();
                    data.forEach(function (effect) {
                        var elt = Mustache.render(template, {
                            id: effect.id,
                            name: effect.name
                        });
                        var $elt = $(elt);
            ...
            var bindItemClick = function ($elt) {
                $elt.click(function (e) {
                    if ($elt.hasClass('disabled'))
                        return;
                    $elt.addClass('disabled');
                    var effect = $elt.data("effect");
                    e.preventDefault();
                    e.stopPropagation();
            ...
            var bindRemoveBtn = function ($elt) {
                var btn = $elt.find('button.remove');
                btn.click(function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                    var ask = window.confirm("Would you really like to remove this effect?");
                    if (!ask)
                        return;
            ...
            var bindEditBtn = function ($elt) {
                var btn = $elt.find('button.edit');
                btn.click(function (e) {
                    e.preventDefault();
                    e.stopPropagation();
            ...
            $btnSave.click(function (e) {
                ...
                var data = Utils_1.default.objectifyForm(form);
                try {
                    var id = $btnSave.data('id');
                    var url = '/manager' + (id ? ('/' + id) : '');
            ...
            var relativePositionFormUserEvent = function (e) {
                var x = NaN, y = NaN;
            ...
            var colorPickerUserDown = false;
            ['mousedown', 'touchstart'].forEach(function (event) {
                $pickerCtrl.get(0).addEventListener(event, function (e) {
                    var position = relativePositionFormUserEvent(e);
                    colorPickerUserDown = true;
                    pickColor(position.x, position.y);
                });
            });
            ['mouseup', 'touchend'].forEach(function () {
                $pickerCtrl.get(0).addEventListener('mouseup', function () {
                    colorPickerUserDown = false;
                });
            });
            ['mouseleave', 'touchleave'].forEach(function (event) {
                $pickerCtrl.get(0).addEventListener(event, function () {
                    colorPickerUserDown = false;
                });
            });
            ['mousemove', 'touchmove'].forEach(function (event) {
            ...
            var pickerSize = parseInt($pickerCtrl.find('>div').css('fontSize'));
            ...
            var lastRequestTime = NaN;
            var sendSimpleColor = function (color, delay, x, y) {
                ...
                var hex = "#" + ("000000" + Utils_1.default.rgbToHex(color[0], color[1], color[2])).slice(-6);
        ...
        var checkForm = function (data) {
            ...
            try {
                ...
                if (!Array.isArray(parsedConfig)) {
                    Utils_1.default.triggerException();
                }
            }
            catch (e) {
            ...
            parsedConfig.forEach(function (config) {
                ['name', 'options'].forEach(function (field) {
...
var $ = __webpack_require__(1);
var Utils_1 = __webpack_require__(0);
var ScheduledEvent_1 = __webpack_require__(6);
var ScriptScheduler = /** @class */ (function () {
    function ScriptScheduler() {
    }
    ScriptScheduler.init = function () {
        $(document).ready(function () {
            /**Selecting dom nodes***/
            ScriptScheduler.$schedulerWindow = $("#scheduler-section");
            ScriptScheduler.scheduleForm = document.getElementById("schedule-form");
            ScriptScheduler.choiceChanger
                = ScriptScheduler.scheduleForm.querySelector("#choice-changer");
            ...
            ScriptScheduler.hoursSelector = ScriptScheduler.scheduleForm.querySelector('#hours');
            ScriptScheduler.minutesSelector = ScriptScheduler.scheduleForm.querySelector('#minutes');
            ...
            ScriptScheduler.oneTimeWeeksDay =
                ScriptScheduler.scheduleForm.querySelector("#onetime-weekday");
            ScriptScheduler.calendarList =
                ScriptScheduler.$schedulerWindow.find("#calendar-list-wrapper")[0];
            ScriptScheduler.initView(); // Build some HTML elements
            ScriptScheduler.bindEvents();
        });
    };
    ScriptScheduler.initView = function () {
        for (var i = 0; i < 24; i++) {
        ...
        $(ScriptScheduler.choiceChanger).on('change', function () {
        ...
        ScriptScheduler.calendar.querySelectorAll(".daytime").forEach(function (dayColumn) {
            for (var i = 0; i < ScriptScheduler.timeSpans; i++) {
    ...
    ;
    ScriptScheduler.bindEvents = function () {
        ScriptScheduler.$btnScheduleManager.on('click', function () {
            ScriptScheduler.show();
        });
        ScriptScheduler.btnAddEvent.addEventListener('click', function (e) {
            e.preventDefault();
            ScriptScheduler.createNewScheduledEvent();
        });
        ScriptScheduler.btnShowCreationForm.addEventListener('click', function (e) {
            e.preventDefault();
            ScriptScheduler.resetForm();
            ScriptScheduler.showCreationForm();
        });
        ScriptScheduler.btnHideCreationForm.addEventListener('click', function (e) {
            e.preventDefault();
    ...
    ;
    ScriptScheduler.showCreationForm = function () {
    ...
    ScriptScheduler.hideCreationForm = function () {
        $(ScriptScheduler.form).removeClass('visible');
        $(ScriptScheduler.btnShowCreationForm).removeClass('hidden');
    };
    ScriptScheduler.show = function () {
        ScriptScheduler.refresh().then(function () {
            Utils_1.default.$mainSectionElt.addClass('blur');
    ...
    ;
    ScriptScheduler.refresh = function () {
        var p1 = new Promise(function (resolve, reject) {
            Utils_1.default.getEffectsList(false).then(function (effects) {
        ...
        var p2 = new Promise(function (resolve, reject) {
            Utils_1.default.getScheduledEventsList(false).then(function (events) {
        ...
        Utils_1.default.showLoader();
        return new Promise(function (resolve, reject) {
            Promise.all([p1, p2]).then(function () {
                Utils_1.default.hideLoader();
                resolve();
            }, function () {
                Utils_1.default.hideLoader();
                reject();
    ...
    ScriptScheduler.createEffectList = function (effects) {
    ...
    ScriptScheduler.createEventsList = function (events) {
        ...
        events.forEach(function (rawEvent) {
            var config = JSON.parse(rawEvent.config);
            var effectId = rawEvent.effectId;
            var id = rawEvent.id;
            var weekDay = config.weekDay;
            var repeated = config.repeated;
            var event = new ScheduledEvent_1.default((parseInt(config.timeStart) - (d.getTimezoneOffset())) % (60 * 24), effectId, weekDay, repeated, id);
            ScriptScheduler.scheduledEvents.push(event);
            ScriptScheduler.createEventView(event, rawEvent.effectName);
        });
    };
    ScriptScheduler.loadEventById = function (id) {
        return new Promise(function (resolve, reject) {
            $.ajax({
                url: '/scheduler/' + id,
                contentType: 'application/json',
    ...
    ScriptScheduler.setEventToForm = function (event) {
        ScriptScheduler.resetForm();
        ...
        if (isNaN(event.weekDay) && event.repeated) {
            event.repeated.forEach(function (val) {
                $(ScriptScheduler.repeatedForm).find('input[value="' + val + '"]')[0].checked = true;
            });
        }
        else {
            $(ScriptScheduler.oneTimeWeeksDay).val(event.weekDay);
        }
        var hours = Math.floor((event.timeStart) / (60));
        var offsetHours = Math.floor((((event.timeStart) - (new Date().getTimezoneOffset())) % (60 * 24)) / 60);
        var minutes = (event.timeStart - (60 * hours));
        ScriptScheduler.hoursSelector.value = offsetHours;
        ScriptScheduler.minutesSelector.value = minutes;
    };
    ScriptScheduler.getWeekDayColumns = function (weekDays) {
        var days = ScriptScheduler.$schedulerWindow[0].querySelectorAll('.daytime');
        var ret = [];
    ...
    ;
    ScriptScheduler.getWeekDayLists = function () {
        return ScriptScheduler.calendarList.querySelectorAll('ul.day-list');
    };
    ;
    ScriptScheduler.createNewScheduledEvent = function () {
        var isRepeated = ScriptScheduler.choiceChanger.value !== 'onetime';
        var effectId = ScriptScheduler.scheduleForm.querySelector("#effect-name").value;
        var repeatedDays = [];
        if (isRepeated) {
            ScriptScheduler.repeatedForm.querySelectorAll('input[type="checkbox"]').forEach(function (current) {
                if (current.checked) {
                    repeatedDays.push(parseInt(current.getAttribute('value')));
                }
            });
        }
        var days = isRepeated ? repeatedDays : ScriptScheduler.oneTimeWeeksDay.value;
        var hours = parseInt(ScriptScheduler.hoursSelector.value);
        var minutes = parseInt(ScriptScheduler.minutesSelector.value);
        if (!days || !days.length) {
        ...
        var editMode = $(ScriptScheduler.form).data('id');
        // Post then resetForm & refresh
        $(ScriptScheduler.btnAddEvent).attr('disabled', 'disabled');
        try {
            var id = editMode ? $(ScriptScheduler.form).data('id') : null;
            var url = '/scheduler' + (id ? ('/' + id) : '');
    ...
    ScriptScheduler.removeScheduledEvents = function (id) {
        Utils_1.default.showLoader();
        $.ajax({
            url: '/scheduler/' + id,
            type: 'DELETE'
    ...
    ScriptScheduler.resetForm = function () {
        $(ScriptScheduler.form).data('id', null);
        $.each($(ScriptScheduler.scheduleForm).find('select'), function (i, elt) {
            $(elt).val($(elt).find('>option').first().attr('value'));
            $(elt).trigger('change');
        });
        $.each($(ScriptScheduler.scheduleForm).find('[type=checkbox]'), function (i, elt) {
            elt.checked = false;
        });
    };
    ;
    ScriptScheduler.eventsPositionTracker = [];
    ScriptScheduler.eventHeightPercent = NaN;
    ScriptScheduler.scheduledEvents = [];
    ScriptScheduler.clearEventsView = function () {
        $(ScriptScheduler.calendar).find('.event').remove();
        $(ScriptScheduler.calendarList).find('.event').remove();
    };
    ScriptScheduler.createEventView = function (event, effectName) {
        var eventWidthPercent = 20;
        var eventHeightPercent = ScriptScheduler.eventHeightPercent;
        var loadEventOnClick = function (event) {
            Utils_1.default.showLoader();
            ScriptScheduler.loadEventById(event.getId()).then(function (event) {
                event.config = event.config;
                var scheduledEvent = ScheduledEvent_1.default.fromRow(event);
        ...
        var create = function (list, repeated) {
            ...
            var effectNameSpan = document.createElement('span');
            ...
            var existingEvents = list.querySelectorAll('li.event');
            var inserted = false;
        ...
        var currentPositionPercent = (event.timeStart / (60 * 24)) * 100;
        if (event.isOneTime()) {
            create(ScriptScheduler.getWeekDayLists()[event.weekDay], false);
        }
        else if (event.isRepeated) {
            var columns = ScriptScheduler.getWeekDayColumns(event.repeated);
            columns.forEach(function (column, index) {
                var weekDay = event.repeated[index];
                create(ScriptScheduler.getWeekDayLists()[event.repeated[index]], true);
            });
        }
    };
    return ScriptScheduler;
}());
exports.default = ScriptScheduler;
